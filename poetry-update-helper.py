...
def resolve_version(package: InnerValues) -> str:
    version = None
    match package:
        case dict():
        ...
        case str():
            version = package
        case _:
            print(f"No version data for {package=}")
...
def as_latest(
    ...
    for package_name, package in contents.items():
        ...
        if normalize_value(package_name) in except_packages:
            continue
        version = resolve_version(package=package)

        if isinstance(package, dict) and "extras" in package:
            p = f"'{package_name}@{version}[{', '.join(package['extras'])}]'"
        else:
            p = f"'{package_name}@{version}'"
        packages.add(p)
